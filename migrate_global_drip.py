...
HISTORY_FILE = f"migration_history_{TARGET_COUNTRY.lower().replace(' ', '_')}.json"
...
# 4. INITIALIZATION
# ==========================================
print_now(f"🚀 Starting Hybrid Engine (Global VIP-Surgical Mode) for {TARGET_COUNTRY}...")
...
def process_album_images(images, official_album_name, global_count, processed_history, session_limit):
    ...
    for img in images:
        ...
        prompt = (
            ...
            f"Analyze this photo from {official_album_name}, {TARGET_COUNTRY}, shot by {AUTHOR} and {PARTNER}. "
            ...
            f"3. Prefer exact place names (town + province/prefecture) over broader regional naming unless the specific location is unknown.\n"
            f"4. Within that pattern, capture specific visual elements (textures, lighting, landmarks) and the local {TARGET_COUNTRY} cultural setting.\n"
            f"5. Example: 'Rustic Stone Architecture and Traditional Tea House in the Historical Village of Gaiman'.\n\n"
            f"STRICT INSTRUCTIONS for the 'description' field:\n"
            f"1. The FIRST sentence MUST begin with the exact place name, region, and subject (e.g., 'Kyoto, Japan: A traditional wooden temple...').\n"
            ...
            f"3. Focus on the atmosphere, technical photography (lighting, depth of field), and local cultural context.\n"
            ...
            f"8. CRITICAL FORMATTING & LENGTH: You MUST write a comprehensive description of 10 to 12 sentences. The 'description' field MUST contain BOTH languages within its single string value. Format it exactly like this using explicit newline characters: [English description (10-12 sentences)] \\n\\n---\\n\\n [{TARGET_LANGUAGE} translation (10-12 sentences)]. Do NOT create a separate JSON key for the translation.\n\n"
        ...
        core_fallback_tags = [TARGET_COUNTRY, "Travel Photography", "Wanderlust", "Travel", "Landscape", "Samuel Jeffery", "Audrey Bergner"]
        ...
        sidecar_dir = f"metadata_sidecars_{TARGET_COUNTRY.lower().replace(' ', '_')}"
...
def run_migration():
    ...
    print_now(f"✅ Initialization Complete for {TARGET_COUNTRY}. Target limit for this session: {SESSION_LIMIT} photos.")
